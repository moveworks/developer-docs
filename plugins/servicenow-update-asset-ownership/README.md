...
- `FILTER_QUERY` **(string)** – Filter users by specific property values.
    - You can filter by **user name**, **user email**, or **department**
        - Example filters:
            - `nameLIKE{{user_name}}` → Returns user details by user name.
            - `email={{email_address}}` → Returns user details by email.
            - `department.nameLIKE{{Department_name}}` → Returns user details by department.
- `sysparm_fields` (string) – Fields to include in the response
