...
name: Notify on Break-Fix Case Updates
...
Reads the case by number to validate its current state before closing.
...
- One-tap **Accept solution** to close a resolved case directly from the notification (handed off to a different conversational process).
- One-tap **Reject solution** that collects a reason from the store employee before reopening the case (handed off to a different conversational process).
