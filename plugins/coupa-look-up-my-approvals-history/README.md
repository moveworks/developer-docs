...
The **“Look Up My Approvals History”** plugin allows employees to instantly review their past approval actions in Coupa — including requisitions, invoices, and expense reports — directly through the Moveworks AI Assistant. Instead of navigating through Coupa, users can quickly retrieve a clean, consolidated history of the items they have approved or rejected within a selected time range.
...
**Note:** To enable access to the Coupa API endpoints used in this plugin, ensure that your Coupa API client has the required OAuth scopes and permissions assigned under:

 **Setup → OAuth2/OpenID Connect Clients → [Your Client]**.
...
- `start_date` – Returns approval records created **on or after** this timestamp (Example: `2024-01-01T00:00:00-07:00`)
- `end_date` – Returns approval records created **on or before** this timestamp (Example: `2024-10-01T00:23:59-07:00`)
- `limit` – Maximum number of approval records per page (default: 100)
- `offset` – Pagination offset for fetching the next batch of records
- `order_by` – Sorting order for results, typically `"desc"` for latest approvals first

### **API #2: Get Approvable Item Details**

```bash
curl --location 'https://<YOUR_INSTANCE>/api/{{approvable_type}}/{{approvable_id}}' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer {{access_token}}'
```
...
- `approvable_type` - The Coupa API resource type. Example values: `invoices`, `requisitions`, `expense_reports`.
- `approvable_id` **-** The unique numeric ID of the approvable item.